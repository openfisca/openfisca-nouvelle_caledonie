...
class credits_impot(Variable):
    ...
    def formula(foyer_fiscal, period):
        ...
        credit_solde_noumea_etc = where(
            ...
            np.ceil(
                (foyer_fiscal("solde_investissements_agrees_noumea_etc", period) * retenue)
                / solde_investissements_agrees
                ),
            0,
            )
        ...
        # retenu_yo
        credit_investissements_agrees_noumea_etc = np.ceil(where(
        ...
        # retenu_yp
        credit_investissements_agrees_autres = np.ceil(where(
        ...
        # retenu_yz
        credit_investissements_agrees_mixtes = np.ceil(where(
        ...
        credit_investissement_productif_industriel = min_(
            min_(
                .15 * foyer_fiscal("investissement_productif_industriel", period), # TODO: parameters
                plaf_50,
                ),
            reliquat_plafond_credits,
            )
